...
(function () {
  if (Hippy.device.platform.OS !== 'ios') return;
  ...
  function disable() {
    enabled = false;
    global.Promise._onHandle = null;
    global.Promise._onReject = null;
  }
  ...
  function enable(options) {
    ...
    var displayId = 0;
    var rejections = {};
    global.Promise._onHandle = function (promise) {
      if (
    ...
    global.Promise._onReject = function (promise, err) {
  ...
  enable({
    ...
    onUnhandled: (id, rejection = {}) => {
      ...
      if (stringValue === '[object Error]') {
        var message = Error.prototype.toString.call(rejection);
        var stack = rejection.stack;
        var warning =
        ...
        console.warn('Possible Unhandled Promise Rejection (id: ' + id + '):');
        var errStr = (rejection && (rejection.stack || rejection)) + '';
        errStr.split('\n').forEach(function (line) {
          console.warn('  ' + line);
        });
      }
      global.Hippy.emit('unhandledRejection', rejection, id);
    },
    onHandled: () => {},
  });
})();
